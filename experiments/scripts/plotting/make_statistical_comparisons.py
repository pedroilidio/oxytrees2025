...
def make_raw_latex_table(data, outdir):
    result = (
        data.drop(columns=["hue"])
        .groupby(["dataset", "estimator", "validation_setting"])
        .mean()
    )
    ...
    *,
    ...
    for metric in y_cols:
        ...
        mean_values = (
            ...
            .groupby(level=[estimator_col, hue_col])
            .mean()
        )
        ...
        mean_ranks = (
            ...
            # .rank(pct=True)
            .rank()
            .groupby([estimator_col, hue_col])
            .mean()
        )

        yield metric, pvalue_crosstable, mean_ranks, mean_values
...
def make_visualizations(
    ...
    n_decimals = int(np.ceil(-np.log10(omnibus_pvalue)))
    pvalue_noexp = int(np.ceil(omnibus_pvalue * 10**n_decimals))
    if pvalue_noexp == 10:
    ...
    assert (
        (pvalue_noexp - 1) * 10 ** (-n_decimals)
        < omnibus_pvalue
        <= pvalue_noexp * 10 ** (-n_decimals)
    )
    ...
    #     ranks = mean_values.droplevel(hue_col)
    # else:
    #     ranks = mean_ranks.droplevel(hue_col)
    ranks = mean_ranks.droplevel(hue_col)

    # HACK
    ranks.index = "\\textbf{" + ranks.index + "}"
    pvalue_crosstable_bold = pvalue_crosstable.copy()
    pvalue_crosstable_bold.index = "\\textbf{" + pvalue_crosstable_bold.index + "}"
    pvalue_crosstable_bold.columns = "\\textbf{" + pvalue_crosstable_bold.columns + "}"
    hue_dict_bold = hue_dict.copy()
    hue_dict_bold.index = "\\textbf{" + hue_dict_bold.index + "}"

    plot_critical_difference_diagram(
        ranks,
        pvalue_crosstable_bold,
        crossbar_props={"marker": "."},
        # label_props={"fontweight": "bold"},
        hue=hue_dict_bold,
        hue_order=hue_dict_bold.index,
        label_fmt_left="\\textbf{{{label} ({rank:.1f})}}",
        label_fmt_right="\\textbf{{({rank:.1f}) {label}}}",
    )
    plt.title(f"Average rank of {formatted_metric_name} ({pvalue_eq})")
    ...
    # HACK
    if data.dataset.iloc[0] == "all_datasets":
        formatted_metric_name = "Ranks of " + formatted_metric_name
        data = data.copy()
...
def make_statistical_comparisons(
    *,
    ...
    allsets_data = (
        ...
        .rank(ascending=False)  # Rank estimators per fold
