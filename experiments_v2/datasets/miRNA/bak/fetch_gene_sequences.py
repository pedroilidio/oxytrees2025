...
async def rate_limited_iterator(iterable, max_per_second: int):
...
async def fetch_gene_table(
    ...
    chunk_iterator = rate_limited_iterator(
        ...
        max_per_second=3,  # Respect NCBI's recommendation of 3 requests per second.
    )
    try:  # Using try-finally to ensure gene_table is saved even if an error occurs
        async for chunk in chunk_iterator:
            new_rows = await fetch_gene(gene_ids=chunk)
            if new_rows is None:
                continue
...
async def fetch_gene_sequences(
    ...
    try:  # Using try-finally to ensure seq_table is saved even if an error occurs
        # tasks = []
        # async with asyncio.TaskGroup() as tg:  # TODO: Python 3.11
        # async for _, gene_data in gene_iterator:
        #     tasks.append(
        #         asyncio.create_task(fetch_nuccore(gene_sequences_output, gene_data))
        #     )
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            tasks = [
                loop.run_in_executor(
                    ...
                    fetch_nuccore,
                    gene_data,
                    gene_sequences_output,
                )
                for _, gene_data in missing_genes.iterrows()
                # async for _, gene_data in gene_iterator
            ]
            await asyncio.gather(*tasks)
    finally:
...
def fetch_nuccore(gene_data, gene_sequences_output):
    ...
    """
    print(f"Fetching sequence for gene {gene_data.GeneID}...")
    try:
        with Entrez.efetch(
            db="nuccore",
            ...
            fasta = handle.read()
        with gene_sequences_output.open("a") as out:
            out.write(f">{gene_data.GeneID} {fasta.removeprefix('>')}\n")
        print(f"Saved sequence for gene {gene_data.GeneID}.")
    except HTTPException as e:
        warnings.warn(
...
async def fetch_gene_sequences_pipeline(
    ...
    gene_table = await fetch_gene_table(gene_entrez_ids, gene_table_output, chunksize)
    seq_table = await fetch_gene_sequences(
